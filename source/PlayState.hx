...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		timeTxt.setFormat(Paths.font("minecraft.ttf"), 20, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("minecraft.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		judgementCounter.setFormat(Paths.font("replacement.ttf"), 30, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (ClientPrefs.hideHud || ClientPrefs.hideJudge) {
		...
		botplayTxt.setFormat(Paths.font("minecraft.ttf"), 28, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		var zincuonnWatermark:FlxText = new FlxText(0, FlxG.height - 44, 0, "zincuonn Engine v" + MainMenuState.zincuonnVersion, 20);
		zincuonnWatermark.setFormat(Paths.font("replacement.ttf"), 25, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		zincuonnWatermark.scrollFactor.set();
		add(zincuonnWatermark);
		zincuonnWatermark.cameras = [camHUD];
		if(ClientPrefs.hideCredits || ClientPrefs.hideHud) {
			zincuonnWatermark.visible = false;
		}

		// And this is for Psych Engine
		var peWatermark:FlxText = new FlxText(0, FlxG.height - 24, 0, "Psych Engine v" + MainMenuState.psychEngineVersion, 20);
		peWatermark.setFormat(Paths.font("replacement.ttf"), 25, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if (!ClientPrefs.hitSound) CoolUtil.precacheSound('tickye');
	...
	{	if (ClientPrefs.spectatorMode)
			{
				FlxTween.tween(camHUD, {alpha:0} ,0.1, { ease: FlxEase.linear, type: FlxTweenType.PERSIST});
				cpuControlled = true;
			}

		if (ClientPrefs.huh)
			{
				FlxTween.tween(camHUD, {angle:camHUD.angle + 5} ,0.5, { ease: FlxEase.bounceIn, type: FlxTweenType.PERSIST});
				FlxTween.tween(camGame, {angle:camGame.angle + 20} ,0.5, { ease: FlxEase.smootherStepInOut, type: FlxTweenType.PERSIST});
				FlxTween.tween(healthBar, {angle:healthBar.angle + 30} ,0.5, { ease: FlxEase.circInOut, type: FlxTweenType.PERSIST});
				FlxTween.tween(scoreTxt, {angle:scoreTxt.angle + 50} ,0.5, { ease: FlxEase.circInOut, type: FlxTweenType.PERSIST});
				{
					{
						playerStrums.forEach(function(spr:FlxSprite)
						{
							spr.angle += spr.angle + 10;
						});
		...
		if (FlxG.keys.justPressed.SPACE)
			{
				boyfriend.playAnim('hey', false, false, 0);
				gf.playAnim('cheer', false, false, 0);
			};
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Score: ' + songScore + ' | Misses: ' + songMisses + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '% | Rating: ' + ratingName + ratingFC;
		} else {
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		{
			accuracyText = new FlxText(0, noteMath + " ms", 20);
			add(accuracyText);

			switch (daRating)
			{
				case "bad" | "shit": // bad and shit
					accuracyText.setFormat(Paths.font("pixel.otf"), 24, 0xFFE52C2C, CENTER, FlxTextBorderStyle.OUTLINE, 0xFFE52C2C);
				case "good": // good
					accuracyText.setFormat(Paths.font("pixel.otf"), 24, 0xFF00D100, CENTER, FlxTextBorderStyle.OUTLINE, 0xFF00D100);
				case "sick": // sick
					accuracyText.setFormat(Paths.font("pixel.otf"), 24, FlxColor.CYAN, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.CYAN);
				case "epic": // epoc
					accuracyText.setFormat(Paths.font("pixel.otf"), 24, 0xFFBB7BFF, CENTER, FlxTextBorderStyle.OUTLINE, 0xFFCF9FFF);
			}
			...
			if (ClientPrefs.hideMS || ClientPrefs.hideHud)
		...
		accuracyText.screenCenter();
		accuracyText.x = comboSpr.x + 100;
		accuracyText.y = comboSpr.y + 100;
		...
		else
		{
			...
			comboSpr.setGraphicSize(Std.int(comboSpr.width * 4));
		}
		...
		for (i in seperatedScore)
		{
			...
			accuracyText.x = comboSpr.x + 5;
			accuracyText.y = numScore.y + 10;
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			...
			if (ClientPrefs.keSustains) // 1.8 input btw
		...
		if (!note.wasGoodHit)
		{
			...
			if (!note.isSustainNote)
			{
				...
				if (ClientPrefs.hitSound) {
					FlxG.sound.play(Paths.sound('tickye', 'shared'));
				}
			}
	...
	override function beatHit()
	{
		...
		//if(ClientPrefs.rotateIcons) {
			...
			/*else if(!ClientPrefs.rotateIcons) {
				if(curBeat % 2 == 0) {
					if (boyfriend.animation.curAnim.name != null && !boyfriend.animation.curAnim.name.startsWith("sing"))
					{
						boyfriend.dance();
				...
				//BUG WITH GF, PLZ HELP FIX IT

				//on second thought, turoial is glitched (bad english)
			}
		}*/
		...
		if(ClientPrefs.huh) {
			if (curBeat % gfSpeed == 0) 
				...
				curBeat % (gfSpeed * 2) == 0 ? 
				{
					iconP1.scale.set(2, 3);
					iconP2.scale.set(-2, -3);
					FlxTween.angle(iconP1, -360, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
					FlxTween.angle(iconP2, 360, 0, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quadOut});
				} 
				: 
				{
					iconP1.scale.set(2, 3);
				...
				FlxTween.tween(boyfriend, {'scale.x': Math.random(), 'scale.y': Math.random()}, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quartInOut, type: FlxTweenType.PERSIST});
				FlxTween.tween(gf, {'scale.x': Math.random(), 'scale.y': Math.random()}, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quartInOut, type: FlxTweenType.PERSIST});
				FlxTween.tween(dad, {'scale.x': Math.random(), 'scale.y': Math.random()}, Conductor.crochet / 1300 * gfSpeed, {ease: FlxEase.quartInOut, type: FlxTweenType.PERSIST});
